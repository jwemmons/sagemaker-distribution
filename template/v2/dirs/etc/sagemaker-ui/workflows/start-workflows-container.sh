...
if [ $V2_CONFIG_ENABLED = "True" ]; then
    ...
    aws s3 sync $WORKFLOW_PLUGINS_SOURCE_S3PATH $WORKFLOW_PLUGINS_PATH
...
if [ -f $USER_STARTUP_FILE ]; then
    if [ $V2_CONFIG_ENABLED = "True" ]; then
        cat $USER_STARTUP_FILE >> "${WORKFLOW_PLUGINS_PATH}/user-startup.sh"
    else
...
if [ -f $USER_REQUIREMENTS_FILE ]; then
    if [ $V2_CONFIG_ENABLED = "True" ]; then
        cat $USER_REQUIREMENTS_FILE >> "${WORKFLOW_PLUGINS_PATH}/user-requirements.txt"
    else
